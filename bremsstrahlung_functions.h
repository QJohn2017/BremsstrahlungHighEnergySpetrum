...
double Elwert_factor(double beta_i, double beta_f, int Z) {
    double value = beta_i * (1. - std::exp(-2. * M_PI * (double)Z * cgs::alpha_f / beta_i));
    value /= beta_f * (1. - std::exp(-2. * M_PI * (double)Z * cgs::alpha_f / beta_f));
    return value;
}
...
double xi_func(double T_electron, double k, int Z, int N) {
    ...
    double xi = 1. + (double)N / pow2(Z) * (1. - std::exp((b - T_electron) / 9. / b));
...
double R_1(double q, double Z) {
    double F_1 = 1. / pow2(1. + pow2(q) / pow2(2. * cgs::alpha_f * (double)Z));
    return 1 - F_1;
}

double R_2(double q, double Z) {
    double F_2 = 1. / pow2(1. + pow2(q) / pow2(2. * cgs::alpha_f * ((double)Z - 5. / 16.)));
    return 2. * (1. - F_2) - (1. - pow2(F_2)) / (double)Z;
}

double gslFunc_I_Phi_1(double q, void * params) {
    gslParams_I_Phi_N prms = *(gslParams_I_Phi_N *)params;
...
double gslFunc_I_Phi_2(double q, void * params) {
    gslParams_I_Phi_N prms = *(gslParams_I_Phi_N *)params;
    double R_N = (prms.N == 1) ? R_1(q, prms.Z) : R_2(q, prms.Z);
    double factor = pow3(q) - 6. * pow2(prms.delta) * q * std::log(q / prms.delta) + 3. * pow2(prms.delta) * q - 4. * pow3(prms.delta);
...
double Phi_1(double gamma_i, double gamma_f, double k, double delta, int Z, int N) {
    ...
    double value = (double)pow2(Z - N) * Phi_u(gamma_i, gamma_f, k) + 8. * Z * (1. - (N - 1.) / (double)Z + I);
    return value;
}

double Phi_2(double gamma_i, double gamma_f, double k, double delta, int Z, int N) {
    double I = I_Phi_2(delta, Z, N);
    double value = (double)pow2(Z - N) * Phi_u(gamma_i, gamma_f, k) + 8. * Z * (5. / 6. * (1. - (N - 1.) / (double)Z) + I);
    return value;
}
